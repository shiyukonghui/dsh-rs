...
| **C8** chat 视图 | ✅ **已落地**（2026-09-05，D-193，切片 C8-1..4） | chat 校验 + `chatFoldFrame`/`chatOptions`（node 26/26）；宿主面复用回正（session.history 既表面 + list/prompt 别名）；`renderChat` 四档点亮；`panel-chat` 声明单元（第八卡，零自有数据端点）；SSE 直订已接（C8-3b，仅订 events.mux）；`.spec/service-assembly-ui-c8-chat/acceptance.md` |
