# 服务装配前端：交接总览（2026-09-05 · 第 64 轮·终态执行后）

一页入口：验证、退役存档与后续观察所需的一切。细节各有专档，此处只做路由。

## 1. 终态一句话（D-212 已执行）
**根 `/` 即服务单元桌布（Dioxus Rust 壳，编译期内嵌）**：旧 deepseek 前端已下线
（SPA 不再服务、`__DSH_BOOT__` 绝迹、index.html 非启动前置），JS 渲染器已退役
（存档 `.spec/archive/canvas-js/`，node 35/35 随迁，canvas.css 幸存）。
旧 SPA 深链直落桌布；未匹配资产 404 不回落。全量审计在根地址全绿。
