// UI 需求取证：原版 harness(3080) 与当前壳(60890) 截图对比。
...
const prof = path.join(os.tmpdir(), `dsh-uishot-${Date.now()}`);
...
const shot = async (url, file, waitMs) => {
  await send("Page.navigate", { url });
  await sleep(waitMs);
  const r = await send("Page.captureScreenshot", { format: "png" });
  const data = r.result?.data;
  if (data) fs.writeFileSync(file, Buffer.from(data, "base64"));
  console.log(file, data ? "OK" : "FAIL");
};
await shot("http://127.0.0.1:3080/", "target/ui-ref/orig-3080.png", 9000);
// 原版可能的画布/桌布路由（若主页非卡片画布，试常见路径）
await shot("http://127.0.0.1:60890/", "target/ui-ref/curr-60890.png", 7000);
proc.kill();
