...
fn spec(argv: Vec<String>, stdin: StdinMode) -> SubprocessSpawnSpec {
    SubprocessSpawnSpec {
        ...
        stdio: ChildStdio {
            stdin,
            stdout: StdoutMode::Collect(SubprocessCollect { max_bytes: 4096, spill: None }),
...
fn cat_argv() -> (String, Vec<String>) {
    #[cfg(windows)]
    {
        ("cmd".to_string(), vec!["/c".to_string(), "findstr".to_string(), "/r".to_string(), ".*".to_string()])
    }
    #[cfg(not(windows))]
    {
        ("cat".to_string(), vec![])
...
fn stdin_write_bytes_reaches_child_then_closes() {
    ...
    assert!(out.contains("line-one"), "stdout carries line-one, got {out:?}");
    assert!(out.contains("line-two"), "stdout carries line-two, got {out:?}");
}

#[test]
fn spawn_failure_is_result_err_not_panicking_handle() {
    let bad = spec(vec!["__definitely_missing_exe__".to_string()], StdinMode::Ignore);
...
fn terminate_kills_running_child() {
    ...
    #[cfg(windows)]
    let argv = vec!["cmd".to_string(), "/c".to_string(), "ping".to_string(), "-n".to_string(), "30".to_string(), "127.0.0.1".to_string()];
    #[cfg(not(windows))]
    ...
    assert_ne!(outcome.exit_code, Some(0), "terminated process must not exit 0");
}
