...
E2E §1 设置编辑行加秘密写烟测（设→list 显示已设→保存留空不变）。撤本提交回到 `4d5057d`。

## D-205（define 终审判级：非目标——旧前端本无此浏览器能力；画布功能平价达成）

**日期**：2026-09-05

**触发问题**：动作级复审第五击——最后一个「不可机械化」候选 define（动态插件新定义）。

**取证（TS 全表面 grep `dynamicCordisRunner/`，9 命中全数审读）**：旧前端该命名空间下
浏览器侧只有**事件**（`package`/`retract`/`cordis/request-run[-resolved]`）与 `invoke`；
README 钉死语义——「host runner 把每个定义的**代码留在进程内存**」，define 经浏览器
半只是**应答 request-run**（要不要运行），**不存在 define RPC**（代码注入是宿主/文件
语义；dsh-rs 对应物 = 动态插件目录 + serve 扫描注册）。

**判级**：canvas 上做 define = 新造代码上传协议（远程代码注入攻击面），**超出「前端改写」
目标边界**——定级**非目标**，从 §2 缺口清单移除非目标清单。旧前端的该项能力本就依赖
嵌入式宿主通道，浏览器形态下从未存在。
...
**回滚点**：撤本提交回 `0f6516a`。教训固化：**新契约字段必须先 live 抓样再造夹具**。

## D-209（可关闭卡片 + 实测高度重排防重叠——Pretext 评估后零依赖落地）

**日期**：2026-09-05

**触发**：用户两点反馈：① 卡片应可 ✕ 关闭、点侧栏标题重开；② 卡片重叠，问用
Pretext 计算布局还是可拖动。

**根因（②）**：绝对定位按**声明** h×格高排位 + `minHeight` 只保底——内容超声明高度
必重叠（layoutGrid 从未知真实高度）。

**选型裁定**：
- **Pretext**（@chenglou/pretext，无 DOM 文本测量引擎）：量的是文字流，卡片是
  input/table 控件盒——量不出控件高度，用武之地错位——**否决**（未来聊天气泡富文本
  预排可再评估）。
- **拖动**：与重叠正交（拖后仍可叠），列后续切片（pointer+localStorage）。
- **采纳**：实测重排——`layoutMeasured` 纯函数（列窗口 masonry，真实 hPx 入位，
  node 测钉死互不重叠）+ ResizeObserver + debounce(200ms) 自动重排（异步 dataRpc
  填充后内容变高自动收敛）。
- **关闭**：卡头 ✕ → closed 集合 **localStorage 持久**（刷新保持）；侧栏语义统一 =
  开着聚焦/关着重开+聚焦；灰显删除线提示；空态文案引导重开。

**验收实测（活体浏览器）**：A) 13 卡**两两矩形零重叠**（程序化断言）；B) ✕ 关闭 →
13→12、侧栏灰显 1、localStorage 命中；C) 点标题重开 → 13、灰显清除；node **35/35**
（红先行）、canvas 守卫 3/3（18 导出）、Console 零错误；截图亲验（全字段完整展示，
...
## D-213 分组桌板：每标题一张桌布（2026-09-05，第 65 轮用户提出并四问拍板）

**触发**：用户「model、config 这些，每个标题都有一个独立的显示区域或者说桌布可以吗」，
随后细化「config 板只显示 config 的卡；点 model 组下标题→切到 model 板」。
**拍板四题**：分组桌板形态｜关闭集每板独立｜自动排布先行（拖拽后置）｜`#board=<组>` 深链。
**选项**：单页分区/混合形态（否决）；关闭保持全局（否决——正是现状缺陷，
「A 板关的卡 B 板也没了」类需求最常踩的坑）；点击灰显卡=跳板重开（否决——
改为**原地重开不跳板**，保住「重开」直觉；正常标题点击才切板+聚焦）。
**实现**：新纯逻辑层 `board.rs`（板→列表 map、close_on/open_on/closed_for/
migrate_legacy，4 测钉隔离语义）；localStorage 升 v2 键，**旧全局键一次性迁移归
「全部」板**（老用户状态不丢）；侧栏卡片项灰显按「视野板」（当前板或所属组原生板）；
hash 读于启动、写于每次切板；hash 指向已消失的组（热插拔后）自动回总览。
**验证**：native 26/26（+4 板级测）；真路由全量审计**新增 T12/T13**：
config 板 3 卡→关 1=2、切回总览 13 卡（**关闭集零牵连实证**）、点 model 卡片标题
切板=1 卡+`#board=model`、`#board=session` 全新加载直达 3 张 session 卡；
T0–T11 原基线不回退、零 console 错误。
**影响与回滚**：canvas-shell（board.rs/interop/app.rs）+ 审计脚本；
revert 内嵌件与本轮源码即回全局关闭语义。

## D-214 拖拽摆位 + 每板摆位记忆（2026-09-05，第 66 轮用户令「按流程完成」）

**触发**：D-213 拍板时明确推迟的后续项，用户下令补齐（需求稿 §8 先落再码）。
**设计决定**：拖标题栏（`.cap`，✕ 除外）→ 钉住（pin），钉位压过自动/声明位；
自动装箱只排未钉卡；板级「⟲ 重置摆位」一键回自动；存储 `dsh.canvas.pos`
（板→卡→{x,y}）与关闭集同风格=**每板记忆零牵连**；位移 >4px 才算拖（防误钉）；
触屏不做（桌面场景）。
**关键防坑（需求稿点名）**：mousemove 绝不写信号——**拖中直改元素 style、松手一次性
落状态+存储**；测量脉冲以 `is_dragging()` 让路，杜绝「拖到一半被重排拽回」。
**实现**：board.rs 纯逻辑（set_pin/pins_of/has_pins/reset_board/merge_pinned，
+3 测钉覆盖/总高/兜底，29/29）；interop 全局委托监听三件套（mousedown/move/up，
AtomicBool 守卫）；app.rs 渲染钉位优先 + 测量回路合并 + 头部重置入口。
**验证**：审计新增 **T14**（CDP Input 真事件拖拽）：movedX=150/movedY=120 与拖距
分毫不差、pinMatchesStyle=true、**cross=0（他板零牵连）**、刷新后钉位仍在、
重置后存储清空+按钮消失；T0–T13 全基线不回退、零 console 错误。
**回滚**：revert 本轮（内嵌件回 D-213 态即无拖拽）。

## D-215 拖拽防重叠：磁吸空格 + 装箱避障（2026-09-05，第 67 轮用户报告+三选一定夺）

**触发**：用户报告「拖拽后卡片可以重叠」并提「停拖 0.5s 自动排布是否合理」。
**需求剖析**：重叠两来源——①松手即压在别的卡上；②自动装箱不认识钉卡，事后
重排（热插拔/改窗宽）未钉卡流进钉区。用户方案「全量自动归位」与 D-214
「摆位记忆」冲突（拖了=白拖），三选项摆开后用户选定 A（磁吸+避障）。
**选项**：B=0.5s 全量自动归位（否决：抹掉钉位记忆）；C=仅碰撞解除不动装箱
（否决：治标不治本，事后重排仍撞）。
**实现（全在可测层）**：`snap_slot`（board.rs 纯逻辑：x 吸附列栅格、y 吸附 10px、
列位由近及远扫首个非相撞、整行全撞沉底重扫、右出界钳回）——**钉前吸附，
钉位永远是干净格**；`layout_measured_avoid`（layout.rs：候选下沉直到避开障碍矩形，
无障碍时=原行为逐位相等）——**钉卡=障碍，装箱绕行**；interop 补
other_card_rects/card_own_size/set_card_pos（薄搬运）。
**验证**：纯逻辑 33/33（+磁吸三测含「满行必下沉」「出界钳回」不变式、+避障一测
含「无障碍=原样逐位相等」）；**T15** CDP 真事件把 A 卡直接压到 B 卡上：
**ovAfterDrop=0 / ovAfterPulse=0**（松手即零重叠 + 脉冲重排后仍零重叠）、pinned=true；
T14 语义随之诚实更新（位移=磁吸槽位差；窄窗 2 列下可下沉寻位，宽窗就近——
需求稿 §9「容忍空洞」假设的预期行为）。
**回滚**：revert 本轮即回自由落位（重叠回归，故不建议）。

## D-216 契约标准化：dsh-std 概念落地 + P0 文法硬切（2026-09-05，第 68-71 轮线）

**触发**：用户问「dsh-std 标准化概念能否用于我们的 wasm 能力」；能力单元化任务
（35 crate 四态分析+地基 8 件套四根判据见 .spec/capability-unitization/）的薄服务族
首刀需要 requires/supports 声明面与挂载前协商。
**选项**：采概念不采依赖（选定）｜vendoring 双实现（否决：TS↔Rust 同步税）｜
方言映射保旧串（否决——用户拍板「全部纯标准文法，不进行兼容性设计」）。
**选择**：①三原语（apiVersion 文法/声明校验/Catalog 协商）Rust 零依赖移植，
文法正则逐字对齐 dsh-std（双实现互认=「采概念」实证）；②**P0 硬切**
`dsh/plugin-ui/v2`→**`dsh.panel-ui/v2`** 全量替换（39 文件：13 ui.json+13 单元
lib.rs+双校验器+测试断言+文档），**旧方言串此后同 v1 同罪**（壳/宿主两校验点
各加拒收测试钉死），major 保持 2（改标识不改契约内容）；③双账本规矩=JSON 声明
是视图、RPC 消费面是真身、CI 对账是装订机（设计期自下而上修正：面板单元经
host-remote 桥 WIT imports 恒同形，对账基准改「声明 ⊇ RPC 消费」）；
④P1-P3（dsh-contract crate/协商关+报告面/plan 试点）按设计稿分期推进。
**验证（P0）**：红先行（拒收测试先失败于旧串仍有效）→ 切换后 shell 33/33、
宿主 lib 263/263、13 组件重建+m32~m41 全 54/54、grep 硬闸（wasm-plugins 命中 0，
全仓仅存 5 处=钉死拒收的测试夹具）、真路由审计 T0–T15 首次全绿零 console。
**回滚点**：P0 单提交 revert（ui.json/lib.rs/校验器同提交原子）；组件 wasm 为本地
构建物随源重建；后续相各自独立回滚（设计稿 §0 表）。
...
## D-224 · canvas-shell 视觉层移植 Semi Design 令牌（UI 靠近原版 harness）

**日期**：2026-09-06 ｜ **触发**：目标「逐一完善插件 UI，靠近 DeepSeek Harness 原版
（React+Semi，浅色）」——现壳为自造深色板，与原版视觉语言（白底发丝边/三档文字/
#0064FA 主色/浅版徽章）差异大。
**选项**：① React 岛/引组件库 → 否决（双运行时，见对话评估）；② 像素复刻 DOM →
否决（伪组件层）；③ **令牌层移植**（选择）：颜色/圆角/字体/控件质感全部换成
semi-theme-default 语义值（源码为权威），DOM 与布局几何（D-181 契约）零改动。
**裁决（R1-R3）**：浅色默认+`body[theme-mode=dark]` 双主题（Semi 官方深色谱）；
质感层深度；系统字体栈。
**实现**：canvas.css 单文件重写（调色板三元组→语义层→使用面；focus 蓝圈；
按钮/输入/表格/徽章/气泡三态；滚动条）。
**关键坑（教训入册）**：CSS 自定义属性 `var()` **在使用处解析**——`:root` 上
以三元组定义的派生变量被继承固化，body 深色块改三元组**不追溯**已派生值；
深色块必须显式重声明全部派生变量（字面量）。表现为深色下 active/徽章仍浅色。
